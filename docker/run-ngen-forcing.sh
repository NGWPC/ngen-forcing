...
show_help() {
  echo "Usage: $(basename "$0") <command> <cycle_name> <gpkg_file> <config_file> <forcing_file> [stdout_file] [conda_env]"
  echo ""
  echo ""
  echo "COMMAND:"
  ...
  echo "FORCING_FILE: Path to the forcing file  to be created."
...
echo "   Running ${CONDA_CMD} $(basename "$SCRIPT_PATH") with: $GPKG_FILE $CYCLE_NAME $CONFIG_FILE $FORCING_FILE"
if [ -z "$STDOUT_FILE" ]; then
  $CONDA_CMD python "${SCRIPT_PATH}" "${CYCLE_NAME}" "${GPKG_FILE}" "-config_input" "${CONFIG_FILE}" "-output_path" "${FORCING_FILE}"
else
  $CONDA_CMD python "${SCRIPT_PATH}" "${CYCLE_NAME}" "${GPKG_FILE}" "-config_input" "${CONFIG_FILE}" "-output_path" "${FORCING_FILE}" &> "${STDOUT_FILE}"
fi
