...
#SBATCH --partition=c5n-18xlarge # the partition
#SBATCH --ntasks-per-node=18     # number of cores per node
#SBATCH --exclusive
...
# Optional: Validate HOT_START_FILE exists if provided
...
#
# location of the NWM retrospective or archived forcing files
...
#
# location of the NWM retrospective or archived streamflow files
# note that the time span of the files must cover the whole simulation period
...
# Validate COASTAL_DOMAIN is a recognized domain
if [[ -z "${coastal_domain_to_inland_domain[$COASTAL_DOMAIN]+x}" ]]; then
    echo "ERROR: Unknown COASTAL_DOMAIN: $COASTAL_DOMAIN (valid: ${!coastal_domain_to_inland_domain[*]})"
    exit 1
fi
...
run_in_container ./run_sing_coastal_workflow_pre_forcing_coastal.bash
...
run_in_container ./run_sing_coastal_workflow_post_forcing_coastal.bash

run_in_container ./run_sing_coastal_workflow_update_params.bash

if [[ $USE_TPXO == "YES" ]]; then
   run_in_container ./run_sing_coastal_workflow_make_tpxo_ocean.bash
else
   export CYCLE_DATE=$STARTPDY
   export CYCLE_TIME=${STARTCYC}00
   export LENGTH_HRS=$(run_in_container ./run_sing_coastal_workflow_pre_make_stofs_ocean.bash)
   ...
   run_in_container ./run_sing_coastal_workflow_post_make_stofs_ocean.bash

fi

run_in_container ./run_sing_coastal_workflow_pre_schism.bash
...
run_in_container ./run_sing_coastal_workflow_post_schism.bash
