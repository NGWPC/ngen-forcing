...
def main(args):
    ...
    if os.path.isfile(lockFile):
        ...
        print(
            "ERROR: Another CFSv2 Fetch Program Running. PID: " + pid + ". Please remove lockfile before attempting to execute another file extraction. Exiting script")
        sys.exit(1)
    else:
    ...
    for hour in range(cleanBackHours, lookBackHours, -1):
        ...
        if dCurrent.hour != 0 and dCurrent.hour != 6 and dCurrent.hour != 12 and dCurrent.hour != 18:
            ...
            cfsCleanDir = outDir + "/cfs." + dCurrent.strftime('%Y%m%d') + "/" + dCurrent.strftime('%H')
            if os.path.isdir(cfsCleanDir):
    ...
    for hour in range(lookBackHours, lagBackHours, -1):
        ...
        if dCurrent.hour != 0 and dCurrent.hour != 6 and dCurrent.hour != 12 and dCurrent.hour != 18:
            ...
            cfsOutDir2 = outDir + "/cfs." + dCurrent.strftime('%Y%m%d') + "/" + dCurrent.strftime('%H')
            if not os.path.isdir(cfsOutDir2):
                os.mkdir(cfsOutDir2)

            cfsOutDir = outDir + "/cfs." + dCurrent.strftime('%Y%m%d') + "/" + dCurrent.strftime('%H') + "/6hrly_grib_" + ensNum
...
def get_options():
    ...
    parser.add_argument('--cleanBackHours', type=int, default=720,
