...
def main(args):
    ...
    for hour in range(cleanBackHours, lookBackHours, -1):
        ...
        fileClean = outDir + "/MultiSensor_QPE_01H_Pass1/" + dCurrent.strftime('%Y%m%d') + "/MRMS_MultiSensor_QPE_01H_Pass1_00.00_" + dCurrent.strftime('%Y%m%d') + \
        ...
        fileClean = outDir + "/MultiSensor_QPE_01H_Pass2/" + dCurrent.strftime('%Y%m%d') + "/MRMS_MultiSensor_QPE_01H_Pass2_00.00_" + dCurrent.strftime('%Y%m%d') + \
            "-" + dCurrent.strftime('%H') + '0000.grib2.gz'
    ...
    for hour in range(lookBackHours,lagBackHours,-1):
        ...
        if os.path.isfile(outFile):
            ...
            timer = 0.0
            print("Pulling MRMS MultiSensor Puerto Rico Pass #1 file: " + url)
        ...
        start_time = time.time()
        timer = 0.0
        print("Pulling MRMS MultiSensor Puerto Rico Pass #2 file: " + url)
        while(download_complete == False and timer < 120.0):
            try:
                request.urlretrieve(url,outFile)
                download_complete = True
