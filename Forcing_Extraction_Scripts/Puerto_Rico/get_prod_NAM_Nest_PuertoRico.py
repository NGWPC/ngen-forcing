...
def main(args):
    ...
    lockFile = os.path.join(outDir, "GET_NAM_Nest_PR.lock")
    ...
    if os.path.isfile(lockFile):
        ...
        print(f"ERROR: Another NAM Nest Puerto Rico Fetch Program running - PID: {pid}.  Please remove lockfile at {lockFile} before attempting to execute another file extraction. Exiting script")
        sys.exit(1)
    else:
    ...
    for hour in range(lookBackHours,lagBackHours,-1):
        ...
        for hrDownload in range(1,nFcstHrs + 1):
            ...
            timer = 0.0
            print("Pulling NAM Nest Puerto Rico file: " + url)
