...
def main(args):
    ...
    if os.path.isfile(lockFile):
        ...
        print(f"ERROR: Another NBM Hawaii Fetch Program running - PID: {pid}.  Please remove lockfile at {lockFile} before attempting to execute another file extraction. Exiting script")
        sys.exit(1)
    else:
    ...
    for hour in range(lookBackHours,lagBackHours,-1):
        ...
        nbmOutDir1 = outDir + "/blend." + dCurrent.strftime('%Y%m%d') + "/core"
        if not os.path.isdir(nbmOutDir1):
            print("Making directory: " + nbmOutDir1)
            os.makedirs(nbmOutDir1)

        nbmOutDir2 = nbmOutDir1 + "/" + dCurrent.strftime('%H')
        ...
        # Request list of NBM Hawaii files in directory since
        ...
        for i in range(len(nbm_urls)):
            ...
            if not os.path.isfile(outFile):
                ...
                timer = 0.0
                print("Pulling Hawaii NBM file: " + nbm_urls[i])
