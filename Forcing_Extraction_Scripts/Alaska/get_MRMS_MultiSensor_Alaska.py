...
def main(args):
    ...
    ncepHTTP = "https://mrms.ncep.noaa.gov/data/2D/ALASKA/MultiSensor_QPE_01H_"

    lockFile = os.path.join(outDir, "GET_Alaska_MRMS_MultiSensor.lock")
    ...
    if os.path.isfile(lockFile):
        ...
        print(f"ERROR: Another MRMS Multisensor Alaska Fetch Program running - PID: {pid}.  Please remove lockfile at {lockFile} before attempting to execute another file extraction. Exiting script")
        sys.exit(1)
    else:
    ...
    for hour in range(cleanBackHours, lookBackHours, -1):
        ...
        fileClean = outDir + "/MultiSensor_QPE_01H_Pass1/" + "/MRMS_MultiSensor_QPE_01H_Pass2_00.00_" + dCurrent.strftime('%Y%m%d') + \
            "-" + dCurrent.strftime('%H') + '0000.grib2.gz'
    ...
    for hour in range(lookBackHours,lagBackHours,-1):
        ...
        if os.path.isfile(outFile):
            ...
            timer = 0.0
            print("Pulling MRMS MultiSensor Alaska Pass #1 file: " + url)
        ...
        start_time = time.time()
        timer = 0.0
        print("Pulling MRMS MultiSensor Alaska Pass #2 file: " + url)
        while(download_complete == False and timer < 120.0):
            try:
                request.urlretrieve(url,outFile)
                download_complete = True
