...
def main(args):
    ...
    if os.path.isfile(lockFile):
        ...
        print(
            f"ERROR: Another MRMS Radar Only CONUS Fetch Program running - PID: {pid}.  Please remove lockfile at {lockFile} before attempting to execute another file extraction. Exiting script")
        sys.exit(1)
    else:
    ...
    for hour in range(cleanBackHours, lookBackHours, -1):
        ...
        fileClean = os.path.join(
            ...
            f"MRMS_RadarOnly_QPE_01H_00.00_{dCurrent.strftime('%Y%m%d')}-{dCurrent.strftime('%H')}0000.grib2.gz"
        )
    ...
    for hour in range(lookBackHours, lagBackHours, -1):
        ...
        radar_dir = os.path.join(outDir, "RadarOnly_QPE",  dCycle.strftime('%Y%m%d'))
