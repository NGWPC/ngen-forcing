...
# conus HRRR 15 minute data (surface files).
...
def main(args):
    ...
    print(f'HRRR subhourly output directory: {outDir}')

    lockFile = os.path.join(outDir, "GET_Conus_HRRR_subhourly.lock")
    ...
    if os.path.isfile(lockFile):
        ...
        print(
            f"ERROR: Another CONUS HRRR SubHourly Fetch Program running - PID: {pid}.  Please remove lockfile at {lockFile} before attempting to execute another file extraction. Exiting script")
        sys.exit(1)
    else:
    ...
    for hour in range(cleanBackHours, lagBackHours, -1):
        ...
        if os.path.isdir(hrrrCleanDir):
            print("Removing old HRRR subhourly data from: " + hrrrCleanDir)
    ...
    for hour in range(lookBackHours, lagBackHours, -1):
        ...
        for hrDownload in range(0, nFcstHrs + 1):
            ...
            timer = 0.0
            print("Pulling HRRR subhourly file: " + url)
