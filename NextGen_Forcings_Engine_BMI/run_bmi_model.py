...
def execute(args):
    ...
    print('\n')
    print('rank:', model._mpi_meta.rank)
    ...
    for x in range(len(ngen_datetimes)):
        ...
        if model._grid_type == "gridded":
            ...
            if model._job_meta.include_lqfrac == 1:
                ...
                print('model time', 'U2D_ELEMENT max', 'V2D_ELEMENT max', 'LWDOWN_ELEMENT max', 'SWDOWN_ELEMENT max', 'T2D_ELEMENT max',
                      'Q2D_ELEMENT max', 'PSFC_ELEMENT max', 'RAINRATE_ELEMENT max', 'LQFRAC_ELEMENT max')
                print(model.get_current_time(), U2D.max(), V2D.max(), LWDOWN.max(), SWDOWN.max(), T2D.max(), Q2D.max(), PSFC.max(), RAINRATE.max(),
                      LQFRAC.max())
                print('model time', 'U2D_ELEMENT min', 'V2D_ELEMENT min', 'LWDOWN_ELEMENT min', 'SWDOWN_ELEMENT min', 'T2D_ELEMENT min',
                      'Q2D_ELEMENT min', 'PSFC_ELEMENT min', 'RAINRATE_ELEMENT min', 'LQFRAC_ELEMENT min')
            ...
            if model._job_meta.include_lqfrac == 1:
                ...
                print('model time', 'U2D_NODE max', 'V2D_NODE max', 'LWDOWN_NODE max', 'SWDOWN_NODE max', 'T2D_NODE max', 'Q2D_NODE max',
                      'PSFC_NODE max', 'RAINRATE_NODE max', 'LQFRAC_NODE max', 'U2D_ELEMENT max', 'V2D_ELEMENT max', 'LWDOWN_ELEMENT max',
                      'SWDOWN_ELEMENT max', 'T2D_ELEMENT max', 'Q2D_ELEMENT max', 'PSFC_ELEMENT max', 'RAINRATE_ELEMENT max', 'LQFRAC_ELEMENT max')
                print(model.get_current_time(), U2D_NODE.max(), V2D_NODE.max(), LWDOWN_NODE.max(), SWDOWN_NODE.max(), T2D_NODE.max(), Q2D_NODE.max(),
                      PSFC_NODE.max(), RAINRATE_NODE.max(), LQFRAC_NODE.max(), U2D_ELEMENT.max(), V2D_ELEMENT.max(), LWDOWN_ELEMENT.max(),
                      SWDOWN_ELEMENT.max(), T2D_ELEMENT.max(), Q2D_ELEMENT.max(), PSFC_ELEMENT.max(), RAINRATE_ELEMENT.max(), LQFRAC_ELEMENT.max())
                print('model time', 'U2D_NODE min', 'V2D_NODE min', 'LWDOWN_NODE min', 'SWDOWN_NODE min', 'T2D_NODE min', 'Q2D_NODE min',
                      'PSFC_NODE min', 'RAINRATE_NODE min', 'LQFRAC_NODE min', 'U2D_ELEMENT min', 'V2D_ELEMENT min', 'LWDOWN_ELEMENT min',
                      'SWDOWN_ELEMENT min', 'T2D_ELEMENT min', 'Q2D_ELEMENT min', 'PSFC_ELEMENT min', 'RAINRATE_ELEMENT min', 'LQFRAC_ELEMENT min')
                print(model.get_current_time(), U2D_NODE.min(), V2D_NODE.min(), LWDOWN_NODE.min(), SWDOWN_NODE.min(), T2D_NODE.min(), Q2D_NODE.min(),
                      PSFC_NODE.min(), RAINRATE_NODE.min(), LQFRAC_NODE.min(), U2D_ELEMENT.min(), V2D_ELEMENT.min(), LWDOWN_ELEMENT.min(),
                      SWDOWN_ELEMENT.min(), T2D_ELEMENT.min(), Q2D_ELEMENT.min(), PSFC_ELEMENT.min(), RAINRATE_ELEMENT.min(), LQFRAC_ELEMENT.min())
            else:
                print('model time', 'U2D_NODE max', 'V2D_NODE max', 'LWDOWN_NODE max', 'SWDOWN_NODE max', 'T2D_NODE max', 'Q2D_NODE max',
                      'PSFC_NODE max', 'RAINRATE_NODE max', 'U2D_ELEMENT max', 'V2D_ELEMENT max', 'LWDOWN_ELEMENT max', 'SWDOWN_ELEMENT max',
                      'T2D_ELEMENT max', 'Q2D_ELEMENT max', 'PSFC_ELEMENT max', 'RAINRATE_ELEMENT max')
                print(model.get_current_time(), U2D_NODE.max(), V2D_NODE.max(), LWDOWN_NODE.max(), SWDOWN_NODE.max(), T2D_NODE.max(), Q2D_NODE.max(),
                      PSFC_NODE.max(), RAINRATE_NODE.max(), U2D_ELEMENT.max(), V2D_ELEMENT.max(), LWDOWN_ELEMENT.max(), SWDOWN_ELEMENT.max(),
                      T2D_ELEMENT.max(), Q2D_ELEMENT.max(), PSFC_ELEMENT.max(), RAINRATE_ELEMENT.max())
                print('model time', 'U2D_NODE min', 'V2D_NODE min', 'LWDOWN_NODE min', 'SWDOWN_NODE min', 'T2D_NODE min', 'Q2D_NODE min',
                      'PSFC_NODE min', 'RAINRATE_NODE min', 'U2D_ELEMENT min', 'V2D_ELEMENT min', 'LWDOWN_ELEMENT min', 'SWDOWN_ELEMENT min',
                      'T2D_ELEMENT min', 'Q2D_ELEMENT min', 'PSFC_ELEMENT min', 'RAINRATE_ELEMENT min')
                print(model.get_current_time(), U2D_NODE.min(), V2D_NODE.min(), LWDOWN_NODE.min(), SWDOWN_NODE.min(), T2D_NODE.min(), Q2D_NODE.min(),
                      PSFC_NODE.min(), RAINRATE_NODE.min(), U2D_ELEMENT.min(), V2D_ELEMENT.min(), LWDOWN_ELEMENT.min(), SWDOWN_ELEMENT.min(),
                      T2D_ELEMENT.min(), Q2D_ELEMENT.min(), PSFC_ELEMENT.min(), RAINRATE_ELEMENT.min())
