...
EV_EWTS_LOGGING = (
    "NGEN_EWTS_LOGGING"  # Enable/disable of Error Warning and Trapping System
)
...
class CustomFormatter(logging.Formatter):
    """Custom logging formatter to adjust level names and use UTC timestamps."""
    ...
    def format(self, record):
        """Format the log record with custom level names and padding."""
...
def create_timestamp(
    date_only: bool = False, iso: bool = False, append_ms: bool = False
) -> str:
    """Create a timestamp string in various formats."""
...
def get_log_file_path():
    """Determine the log file path based on environment variables or defaults."""
    appendEntries = True
...
def translate_ngwpc_log_level(ngwpc_log_level: str) -> str:
    """Translate ngwpc log levels to Python logging levels."""
...
def log_level_set():
    ...
    if loggingEnabled == False:
        ...
        logging.disable(
            logging.CRITICAL
        )  # Disables all logs at CRITICAL and below (i.e., everything)
    else:
        ...
        formatted_module = MODULE_NAME.upper().ljust(LOG_MODULE_NAME_LEN)[
            :LOG_MODULE_NAME_LEN
        ]
