...
def s3_access(ConfigOptions):
    ...
    if ConfigOptions.nwm_domain == 'CONUS':
        ...
        files = [s3fs.S3Map(root=ConfigOptions.nwm_url.format(source=ConfigOptions.nwm_source, domain=ConfigOptions.nwm_domain),
                           s3=_s3, check=False, )]
    return files
...
def get_time_index(zarr_sample, target_time):
    ...
    reference_time = pd.Timestamp(zarr_sample['time'].attrs['units'].split('since ')[-1])
...
def read_zarr_data(files, spatial_bounds, target_index, actual_time, x_coords, y_coords):
    ...
    for i, file_map in enumerate(files):
        ...
        if zarr_var_name in zarr_group:
            ...
            if spatial_bounds is not None:
                y_min_idx, y_max_idx, x_min_idx, x_max_idx = spatial_bounds
                var_data = var_zarr[target_index, y_min_idx:y_max_idx, x_min_idx:x_max_idx]
            else:
            ...
            var_data_with_time = np.expand_dims(var_data_scaled, axis=0)  # Add time dimension at front
...
def proc_nwm(ConfigOptions, MpiConfig):    
    ...
    ''' Helper function to orchestrate processing functions and create output dataset'''
        ...
        with dask.config.set(scheduler=executor):
            if MpiConfig.rank == 0:
                try:
                    ...
                    spatial_bounds, x_coords, y_coords = get_spatial_bounds(zarr_sample, domain_bounds)
                    data_vars, coords, attrs = read_zarr_data(files, spatial_bounds, target_index, actual_time, x_coords, y_coords)
