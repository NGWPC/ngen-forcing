        ...
        if input_forcings.nx_global is None or input_forcings.ny_global is None:
            ...
            if config_options.grid_type == "gridded":
                try:
                    ...
                    input_forcings.esmf_grid_in = ESMF.Grid(
                ...
                try:
                    input_forcings.esmf_grid_in = ESMF.Mesh(
                        filename=config_options.geogrid,
                        filetype=ESMF.FileFormat.ESMFMESH,
                    )
                    input_forcings.esmf_grid_in_elem = ESMF.Mesh(
                        filename=config_options.geogrid,
                        filetype=ESMF.FileFormat.ESMFMESH,
                    )
                except ESMF.ESMPyException as esmf_error:
...
def _regrid_ak_ext_ana_pcp_stage4(
    ...
    try:
        ...
        if config_options.grid_type == "gridded":
            ...
            try:
                supplemental_precip.esmf_field_out = supplemental_precip.regridObj(
    ...
    try:
        ...
        for force_count, grib_var in enumerate(input_forcings.grib_vars):
            ...
            if config_options.grid_type == "gridded":
                ...
                try:
                    input_forcings.esmf_field_out_elem = input_forcings.regridObj_elem(
                        input_forcings.esmf_field_in_elem,
                        input_forcings.esmf_field_out_elem,
                    )
                except ValueError as ve:
...
def regrid_gfs(input_forcings, config_options, wrf_hydro_geo_meta, mpi_config):
    ...
    try:
        ...
        if reuse_prev_file:
            if mpi_config.rank == 0:
                config_options.statusMsg = (
                    "Reusing previous input file: " + input_forcings.file_in2
                )
                err_handler.log_msg(
                    config_options, mpi_config, True
                )  # log at debug level
                ...
                for force_count, grib_var in enumerate(input_forcings.grib_vars):
                    ...
                    if grib_var == "PRATE":
                        ...
                        if input_forcings.fcst_hour2 <= 384:
                            ...
                            diff_tmp = (
                                tmp_hr_current % 6 if tmp_hr_current % 6 > 0 else 6
                            )
...
def regrid_mrms_precip_flag(
    ...
    try:
        supplemental_precip.esmf_field_out = supplemental_precip.regridObj(
            supplemental_precip.esmf_field_in, supplemental_precip.esmf_field_out
        )
    except ValueError as ve:
...
def regrid_sbcv2_liquid_water_fraction(
    ...
    if config_options.grid_type == "gridded":
        ...
        try:
            supplemental_forcings.esmf_field_out = supplemental_forcings.regridObj(
...
def regrid_hourly_nbm(
    ...
    for force_count, nc_var in enumerate(forcings_or_precip.netcdf_var_names):
        ...
        if config_options.grid_type == "gridded":
            ...
            try:
                forcings_or_precip.esmf_field_out = forcings_or_precip.regridObj(
                    forcings_or_precip.esmf_field_in, forcings_or_precip.esmf_field_out
                )
            except ValueError as ve:
...
def calculate_weights(
    ...
    """Calculate ESMF weights based on the output ESMF.
    ...
    if config_options.weightsDir is not None:
        ...
        if os.path.exists(weight_file):
            ...
            try:
                ...
                begin = time.monotonic()
                input_forcings.regridObj = ESMF.RegridFromFile(
                ...
                config_options.errMsg = (
                    "Unable to load cached ESMF weight file: " + str(esmf_error)
                )
                err_handler.log_warning(config_options, mpi_config)

        # check if unstructured mesh element weight file exists:
        if config_options.grid_type == "unstructured" and os.path.exists(
            weight_file_elem
        ):
            # read the data
            try:
                ...
                begin = time.monotonic()
                input_forcings.regridObj_elem = ESMF.RegridFromFile(
                    input_forcings.esmf_field_in_elem,
                    input_forcings.esmf_field_out_elem,
                    weight_file_elem,
                )
                ...
                config_options.errMsg = (
                    "Unable to load cached ESMF mesh element weight file: "
                    + str(esmf_error)
                )
                err_handler.log_warning(config_options, mpi_config)
    ...
    if input_forcings.regridObj is None:
        ...
        try:
            begin = time.monotonic()
            input_forcings.regridObj = ESMF.Regrid(
                input_forcings.esmf_field_in,
                input_forcings.esmf_field_out,
            ...
            config_options.errMsg = "Unable to regrid input data from ESMF: " + str(
                esmf_error
            )
            err_handler.log_critical(config_options, mpi_config)
    ...
    if (
        ...
        and input_forcings.regridObj_elem is None
    ):
        if mpi_config.rank == 0:
            config_options.statusMsg = (
                "Creating ESMF mesh element weight object from ESMF"
            )
            err_handler.log_msg(config_options, mpi_config, True)  # log at debug level
        err_handler.check_program_status(config_options, mpi_config)
        extrap_method = ESMF.ExtrapMethod.CREEP_FILL if fill else ESMF.ExtrapMethod.NONE
        ...
        try:
            begin = time.monotonic()
            input_forcings.regridObj_elem = ESMF.Regrid(
                input_forcings.esmf_field_in_elem,
                input_forcings.esmf_field_out_elem,
                src_mask_values=np.array([0, config_options.globalNdv]),
                regrid_method=regrid_method,
            ...
            config_options.errMsg = (
                "Unable to regrid input data into ESMF mesh element weight object from ESMF: "
                + str(esmf_error)
            )
            err_handler.log_critical(config_options, mpi_config)
            etype, value, tb = sys.exc_info()
        ...
        try:
            input_forcings.esmf_field_out_elem = input_forcings.regridObj_elem(
                input_forcings.esmf_field_in_elem, input_forcings.esmf_field_out_elem
            )
        except ValueError as ve:
            config_options.errMsg = (
                "Unable to extract regridded data from ESMF regridded field: " + str(ve)
...
def calculate_supp_pcp_weights(
    ...
    try:
        # noinspection PyTypeChecker
        supplemental_precip.esmf_grid_in = ESMF.Grid(
    ...
    if config_options.grid_type == "gridded":
        ...
        supplemental_precip.regridObj = ESMF.Regrid(
            supplemental_precip.esmf_field_in,
            supplemental_precip.esmf_field_out,
        ...
        supplemental_precip.regridObj_elem = ESMF.Regrid(
            supplemental_precip.esmf_field_in_elem,
            supplemental_precip.esmf_field_out_elem,
            src_mask_values=np.array([0]),
            regrid_method=ESMF.RegridMethod.BILINEAR,
        ...
        # Run the regridding object on this test dataset. Check the output grid for
        # any 0 values.
        supplemental_precip.esmf_field_out_elem = supplemental_precip.regridObj_elem(
            supplemental_precip.esmf_field_in_elem,
            supplemental_precip.esmf_field_out_elem,
        )
